...
const Dashboard = ({ onLoginClick }) => {
    ...
    // Dashboard.js (ЗАМЕНИТЕ СУЩЕСТВУЮЩИЕ БЛОКИ АВТООБНОВЛЕНИЯ НА ЭТОТ)

    // ... (оставьте все остальные useEffects, кроме автообновления)

    // ✅ НОВЫЙ ИСПРАВЛЕННЫЙ ЭФФЕКТ ДЛЯ АВТООБНОВЛЕНИЯ
    useEffect(() => {
        // Условие: не должно быть открытых модальных окон
        const shouldRunRefresh = !selectedEquipment && !showLaunchConfirm;

        // Если есть открытое модальное окно, мы просто выходим (интервал не запускается)
        if (!shouldRunRefresh) {
            console.log('🛑 Автообновление приостановлено: Открыто модальное окно.');
            return;
        }

        // Если нет открытых модальных окон, запускаем интервал
        const interval = setInterval(() => {
            console.log('🔄 Автообновление данных...');
            refreshData();
        }, 30000); // 30 секунд

        // Функция очистки (вызывается при unmount или при изменении зависимостей)
        return () => {
            console.log('🧹 Очистка интервала автообновления.');
            clearInterval(interval);
        };

        // Таймер теперь перезапускается (очищается и устанавливается заново) 
        // при изменении этих состояний:
